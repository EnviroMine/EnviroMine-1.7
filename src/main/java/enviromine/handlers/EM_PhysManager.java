...
public class EM_PhysManager
{
	...
	public static boolean isLegalType(World world, int x, int y, int z)
	{
		...
		{
			if(!(block instanceof BlockMobSpawner) && !(block instanceof BlockLadder) && !(block instanceof BlockWeb) && !(block instanceof BlockGlowstone) && !(block instanceof BlockSign) && !(block instanceof BlockBed) && !(block instanceof BlockDoor) && !(block instanceof BlockFalling) && !(block instanceof BlockPortal) && !(block instanceof BlockEndPortal) && !(block == Blocks.end_stone) && !(block instanceof BlockEndPortalFrame) && !(block.getMaterial() == Material.vine) && !blockNotSolid(world, x, y, z, false) && block.getBlockHardness(world, x, y, z) != -1F)
			{
				int stabNum = getDefaultStabilityType(block);
